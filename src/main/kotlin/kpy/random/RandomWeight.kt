...
internal class RandomWeight(weights: List<Int>) {
    ...
    private fun binSearch(key: Int): Int {
        ...
        while (lo < hi) {
            ...
            if (summedWt[mid] == key || summedWt[mid] > key && mid - 1 >= 0 && summedWt[mid - 1] < key) break else if (summedWt[mid] < key && summedWt[mid + 1] >= key) {
                mid += 1
                break
            } else if (summedWt[mid] < key) lo = mid + 1 else hi = mid - 1
